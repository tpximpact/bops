...
module BopsApi
  module Comment
    class QuerySpecialistService
      ...
      def select_sql
        "consultee_responses.*,
          consultee_responses.redacted_response,
          ts_rank(
            to_tsvector('english', consultee_responses.redacted_response),
            to_tsquery('english', ?)
        ) AS rank"
      end

      def where_sql
        "to_tsvector('english', consultee_responses.redacted_response) @@ to_tsquery('english', ?)"
      end
